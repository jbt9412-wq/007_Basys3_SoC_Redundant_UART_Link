...
module frame_fifo (
    ...
    always @(posedge clk or posedge reset_p) begin
        if (reset_p) begin
            ...
            frame_length_mem[0] <= 8'd0;
            frame_length_mem[1] <= 8'd0;
            device_id_mem[0]    <= 8'd0;
            device_id_mem[1]    <= 8'd0;
            command_mem[0]      <= 8'd0;
            command_mem[1]      <= 8'd0;
            sequence_mem[0]     <= 8'd0;
            sequence_mem[1]     <= 8'd0;
            payload_data_mem[0] <= 128'd0;
            payload_data_mem[1] <= 128'd0;
            received_crc_mem[0] <= 16'd0;
            received_crc_mem[1] <= 16'd0;
            seq_gap_mem[0]      <= 1'b0;
            seq_gap_mem[1]      <= 1'b0;
        end
        else begin
            ...
            // Push가 허용되면 현재 Tail 위치에 모든 필드를 함께 저장한다.
